...
import { formatManilaYmd, formatSlotRangeLabel, manilaSlotStartUtc, SLOT_HOURS } from "@/lib/manila";
...
function HomeComponent() {
  ...
  const byHour = useMemo(() => {
    const map = new Map<number, { id: string; addr: string | null; noShow?: boolean | null }>();
  ...
  return (
    <div className="container mx-auto max-w-lg px-4 py-6">
      ...
      <p className="mb-6 text-sm text-muted-foreground">
        Hourly schedule · Philippines time (UTC+8) · 06:00–16:00
      </p>
          ...
          return (
            <Card key={hour} className="flex flex-row items-center justify-between p-4">
              <div>
                ...
                <div className="text-xs text-muted-foreground">Asia/Manila</div>
              </div>
      ...
      <p className="mt-6 text-center text-xs text-muted-foreground">
        ...
        (four parties per hour, 16:00–22:00 Manila).
      </p>
    </div>
  );
}
